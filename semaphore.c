...
int fct_sempetunia()
{
    ...
    if ((semid=semget(token,1,0666 | IPC_CREAT)<0)){  //préférable d'utiliser un ftoken ... peut être !
...
void processusEnfant(int numProcessus)
{
  ...
  /**  printf("voiture numero : %d\n", shm_Pt[numProcessus].numero);
    printf("La vitesse :  %lf\n", shm_Pt[numProcessus].vitesse);
    printf("Distance parcouru %lf\n", shm_Pt[numProcessus].distance);
    printf("Number Tour : %d\n", shm_Pt[numProcessus].nbTour);
    printf("Number PIT : %d\n", shm_Pt[numProcessus].pit);
    printf("Numbertour : %5.2lf\n", shm_Pt[numProcessus].chrono.Tour);**/
...
void creerEnfants(int nbEnfants) /**changer nom anti foutre Seb Et shubba dasn la merde**/
...
int iii=0;
   //int x=0;
      ...
      //  printf("eeeeeeeeeeeeeeeeeeee%d,",cpt);
      if ((p = fork()) == 0) {
...
//           printf("eeeeeeeeeeeeeeeekkkcceeee%d,",cpt);
          tabPidEnfants[cpt] = p;
          usleep(55000);
